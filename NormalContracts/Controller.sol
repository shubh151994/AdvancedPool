...
contract Controller {
    ...
    mapping(address => uint256) public availableCRV;
...
/****OPEN FUNCTIONS****/
    ...
    function claimCRV() external onlyStrategy() returns(uint256){
        ...
        uint256 crvToSend = availableCRV[msg.sender] + crvReceived - crvToLock;
        availableCRV[msg.sender] = 0;
    ...
    function releaseLock() external{
        ...
        uint256 crvReceived = newBalance - oldBalance;
        uint256 crvToLock = crvReceived * crvLockPercent / DENOMINATOR;
        uint256 crvPerStrategy = (crvReceived - crvToLock)/totalStrategies;
        for(uint8 i = 0 ; i < totalStrategies; i++){
            availableCRV[depositStrategies[i]] = availableCRV[depositStrategies[i]] + crvPerStrategy;
        }
